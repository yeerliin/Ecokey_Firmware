...
static const estado_entry_t* buscar_estado_entry(estado_app_t estado) {
    ...
    for (size_t i = 0; i < NUM_ESTADOS; ++i) {
        ...
        if (tabla_estados[i].estado == estado) return &tabla_estados[i];
    }
...
static esp_err_t detener_estado(estado_app_t estado) {
    // Busca el estado en la tabla
    const estado_entry_t *entry = buscar_estado_entry(estado);
    // Si el estado existe y tiene función de detención
    if (entry && entry->detener) {
        // Registra el mensaje de detención
        ESP_LOGI(TAG, "Deteniendo estado %s...", entry->nombre);
        // Llama a la función de detención del estado
        return entry->detener();
    }
    // Si no encuentra el estado, registra advertencia
    ESP_LOGW(TAG, "Estado actual no reconocido: %d", estado);
    // Devuelve OK aunque no hizo nada (no es un error crítico)
    return ESP_OK;
}
...
static esp_err_t iniciar_estado(estado_app_t estado) {
    // Busca el estado en la tabla
    const estado_entry_t *entry = buscar_estado_entry(estado);
    // Si el estado existe y tiene función de iniciación
    if (entry && entry->iniciar) {
        // Registra el mensaje de iniciación
        ESP_LOGI(TAG, "Iniciando estado %s...", entry->nombre);
        // Llama a la función de iniciación del estado
        return entry->iniciar();
    }
    // Si no encuentra el estado, registra error
    ESP_LOGE(TAG, "Nuevo estado desconocido: %d", estado);
    // Devuelve error ya que no puede iniciar un estado desconocido
    return ESP_FAIL;
}

/**
 * Cambia el estado actual del sistema, deteniendo el anterior e iniciando el nuevo.
 * Guarda el nuevo estado en NVS.
 * 
...
esp_err_t app_control_cambiar_estado(estado_app_t nuevo_estado) {
    ...
    if (nuevo_estado == estado_actual && estado_inicializado) {
        ESP_LOGW(TAG, LOG_PREFIX_STATE " Estado %d ya activo e inicializado", estado_actual);
        return ESP_OK;
    }
    ...
    if (es_primer_arranque) {
        ESP_LOGI(TAG, LOG_PREFIX_BOOT " Primer arranque o reinicio detectado → Estado %d", nuevo_estado);
    } else {
        ESP_LOGI(TAG, LOG_PREFIX_TRANS " Transición %d → %d (inicializado: %s)", 
                estado_actual, nuevo_estado, estado_inicializado ? "sí" : "no");
    }

    // Si hay un estado anterior válido y está inicializado, lo detenemos
    if (estado_actual != ESTADO_INVALIDO && estado_inicializado) {
        const estado_entry_t *entry = buscar_estado_entry(estado_actual);
        ESP_LOGI(TAG, LOG_PREFIX_STATE " Deteniendo %s (%d)", 
                entry ? entry->nombre : "DESCONOCIDO", estado_actual);
    ...
    ESP_LOGI(TAG, LOG_PREFIX_NVS " Guardando estado %d en NVS", estado_actual);
    app_control_guardar_estado();

    // Inicia el nuevo estado
    const estado_entry_t *entry = buscar_estado_entry(nuevo_estado);
    ESP_LOGI(TAG, LOG_PREFIX_STATE " Iniciando %s (%d)", 
            entry ? entry->nombre : "DESCONOCIDO", nuevo_estado);
    
    resultado = iniciar_estado(nuevo_estado);

    // Maneja el resultado de la iniciación
    if (resultado != ESP_OK) {
        ESP_LOGE(TAG, LOG_PREFIX_STATE " Error al iniciar %s (%d): %s", 
                entry ? entry->nombre : "DESCONOCIDO", nuevo_estado, esp_err_to_name(resultado));
        estado_inicializado = false;
    } else {
        estado_inicializado = true;
        ESP_LOGI(TAG, LOG_PREFIX_STATE " %s (%d) inicializado correctamente",
                entry ? entry->nombre : "DESCONOCIDO", nuevo_estado);
    }
...
esp_err_t app_control_guardar_estado(void) {
    ...
    if (err != ESP_OK) {
        ...
        // Solo verificamos si está en modo debug para reducir verbosidad
        #if LOG_LOCAL_LEVEL >= ESP_LOG_DEBUG
        uint8_t valor_verificacion = nvs_manager_get_u8(NVS_KEY_ESTADO, ESTADO_INVALIDO);
        ESP_LOGD(TAG, LOG_PREFIX_NVS " Verificación: valor leído de NVS: %u", valor_verificacion);
        #endif
    }
...
static void tarea_control_estado(void *param) {
    ...
    while (1) {
        if (xQueueReceive(transicion_queue, &args, portMAX_DELAY) == pdTRUE) {
            const estado_entry_t *entry = buscar_estado_entry(args.destino);
            ESP_LOGI(TAG, LOG_PREFIX_TRANS " Solicitada transición a %s (%d) desde '%s'", 
                    entry ? entry->nombre : "DESCONOCIDO", args.destino, args.tag);
...
esp_err_t app_control_lanzar_transicion(estado_app_t destino, const char *tag) {
    ...
    if (!transicion_queue) {
        ESP_LOGI(TAG, LOG_PREFIX_BOOT " Inicializando subsistema de transiciones");
    ...
    const estado_entry_t *entry = buscar_estado_entry(destino);
    ESP_LOGI(TAG, LOG_PREFIX_TRANS " Encolando transición a %s (%d) desde '%s'", 
            entry ? entry->nombre : "DESCONOCIDO", destino, args.tag);
